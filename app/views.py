...
def get_centrifugo_data(user_id):
    token_expire_seconds = int(os.getenv('CENTRIFUGO_TOKEN_EXPIRE_SECONDS', '6000'))
...
def paginate(objects_list, request, per_page=None):
    if per_page is None:
        per_page = int(os.getenv('PAGINATE_PER_PAGE', '20'))
...
def toggle_correct_answer(request):
    ...
    try:
        ...
        return JsonResponse({'error': 'Question or answer not found'}, status=404)

    if question.user != request.user:
        return JsonResponse({'error': 'Only the author of the question can change the correctness of the answer'}, status=403)
